...
# ============================================================================
def handle_policy_engine_gateway_association(event: Dict[str, Any], request_type: str, properties: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if request_type == 'Delete':
        ...
        if gateway_id:
            try:
                ...
                return send_response(event, 'FAILED', f"Error associating policy engine with gateway: {str(e)}")
        else:
...
def create_policy_engine(event: Dict[str, Any], properties: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        create_response = client.create_policy_engine(
            ...
            description=policy_engine_description
        )
        ...
        try:
            ...
            try:
                response = client.update_policy_engine(
                    policyEngineId=policy_engine_id,
                    description=properties.get('Description', f'Policy Engine: {properties.get("PolicyEngineName", "default_policy_engine")}')
                )
                wait_for_policy_engine_active(client, policy_engine_id)
